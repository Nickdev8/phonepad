...
async function runPhonePadCommand(rawArgs) {
  ...
  if (args[0] === 'server') {
    ...
    console.error('Run `node src/server.js` (or docker compose) on the server host instead.');
    process.exit(1);
  }
  ...
  const childProcess = spawn(process.execPath, [path.join(__dirname, '..', 'src', 'phonepad-client.js'), parsed.baseUrl, adminAccessToken], {
    stdio: 'inherit',
    env: {
      ...process.env,
